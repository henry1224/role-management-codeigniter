                        ...
                        <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <select class="form-control select2" name="id_customer" id="customer">
                                            ...
                                            <?php foreach ($customer as $tampil) { ?>
                                                <option alamat="<?= $tampil->alamat_customer; ?>" value="<?php echo $tampil->id_customer; ?>" <?= $tampil->id_customer == $report->id_customer ? "selected" : null ?>><?php echo $tampil->nama_customer; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row"><label class="col-sm-2 col-form-label">Alamat Customer</label>
                                    <div class="col-md-6"><input type="text" name="alamat_customer" id="alamat_cust" value="<?= $report->alamat_customer; ?>" class="form-control" autocomplete="off" autofocus></div>
                                </div>
                                <div class="form-group row" id="tanggal"><label class="col-sm-2 col-form-label">Tanggal</label>
                                    <div class="col-md-6">
                                        <div class="input-group date">
                                            <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input type="text" name="tanggal_so" value="<?php echo date('d-m-Y',strtotime($report->tanggal_so)); ?>" class="form-control" readonly=autocomplete="off">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                ...
                                <div class="form-group row"><label class="col-sm-2 col-form-label">NO.INVOICE</label>
                                    <div class="col-md-6"><input type="text" name="invoice" id="_invoice" value="<?= $report->invoice; ?>" class="form-control" autocomplete="off" placeholder="Contoh: 023950">
                                    </div>
                                </div>
                                <div class="form-group row"><label class="col-sm-2 col-form-label">Faktur Pajak</label>
                                    <div class="col-md-6"><input type="text" name="faktur_pajak" value="<?= $report->faktur_pajak; ?>" class="form-control" autocomplete="off" autofocus></div>
                                </div>
                                <div class="form-group row"><label class="col-sm-2 col-form-label">NO.PO</label>
                                    <div class="col-md-6"><input type="text" name="no_purchase" value="<?= $report->no_purchase; ?>" class="form-control" autocomplete="off" autofocus></div>
                                </div>
                                <div class="form-group row"><label class="col-sm-2 col-form-label">Surat Jalan</label>
                                    <div class="col-md-6"><input type="text" name="surat_jalan" value="<?= $report->surat_jalan; ?>" class="form-control" autocomplete="off"></div>
                                </div>
                                <div class="form-group row"><label class="col-sm-2 col-form-label">Type PPN</label>
                                    <div class="col-md-6">
                                        ...
                                        <option <?php if ($report->type_ppn == 'I') {
                                                        echo "selected";
                                                    } ?> value="I">PPN INCLUDE</option>
                                            <option <?php if ($report->type_ppn == 'E') {
                                                        echo "selected";
                                                    } ?> value="E">PPN EXCLUDE</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
<script src="<?php echo base_url('assets/js/jquery-3.1.1.min.js'); ?>"></script><script>
    ...
    $('#tambah_data').click(function() {
        ...
        $('#data_table tbody:last-child').append(
            ...
            '<td>' + harga_jual + '</td>' +
            '<td>' + discount + '</td>' +
            '<td id="total_' + nom + '" class="_hit">' + total + '</td><input type="text" name="item[]" value="'+ invoice +'|' + kode + '|' + type + '|' + brand + '|' + pola + '|' + jumlah + '|' + harga_jual + '|' + discount + '|' + total + '"/>' +
